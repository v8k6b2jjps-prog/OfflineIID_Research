#include "pch.h" // Pulls in windows.h, std algorithms, vectors, strings, etc.
...
static __int64 BuildAndEncryptCipherBlock(const uint8_t* pKeyDataStruct, int64_t hwid, std::vector<uint8_t>& outCipherBlock) {
...
static DecodedParameters ReadBackParameters(const std::vector<uint8_t>& plaintext22) {
    DecodedParameters params = { 0, 0, 0, 0 };
    if (plaintext22.size() < 22) return params;
    ...
    params.securityID = secIdLow | (static_cast<uint32_t>(secIdByte2) << 16) | (static_cast<uint32_t>(secIdNibble) << 24);

    for (int i = 0; i < 2; ++i) {
        uint8_t reconstructedByte = static_cast<uint8_t>(
            (plaintext22[5 + i] >> 1) | ((plaintext22[6 + i] & 0x01) << 7)
            );
        reinterpret_cast<uint8_t*>(&params.groupID)[i] = reconstructedByte;
    }
    ...
    uint8_t byte3_high = static_cast<uint8_t>(plaintext22[10] >> 5);
    uint8_t byte3_low = static_cast<uint8_t>((plaintext22[11] & 0x07) << 3);
    reinterpret_cast<uint8_t*>(&var50)[3] = static_cast<uint8_t>(byte3_high | byte3_low);
    ...
    std::memcpy(&params.hwid, hwidBytes, 8);
