...
$HwidModulePath = Join-Path $PSScriptRoot "Hwid_Module.ps1"
try { [BinaryKey] } catch { . $HwidModulePath } Finally { Clear-Host }

$pidgenInsider = Join-Path $PSScriptRoot "pidgenxIn.dll"
$MsftHelperDll = Join-Path $PSScriptRoot "MsftHelperDll.dll"
[Assembly]::LoadFile((Join-Path $PSScriptRoot "Msft2009.dll")) | Out-Null
