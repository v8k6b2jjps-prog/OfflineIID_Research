...
if ($InstallationIdList) {
    ...
    $readResult = [MSFT]::ReadParametersFromString($generatedIid, [ref]$decodedParams);
    if ($readResult -eq 0) {
...
# using Byte Pattern { 0x18, 0x01, 0x00, 0x00 }
if ($sub -le 0) {
  try {
...
if ($sub -le 0) {
  throw "Could not Find offset for HwidGetCurrentEx"
}
...
if ($hr -ge 0 -and $params[2].Value -ne 0L) {
    ...
    $ShortHWID = Hwid-ConvertToLargeInt -Raw $byteArray
    Write-Host "WinRT HWID Value   :  $ShortHWID"
}

# Windows 10-11 Education Key
# NW6C2-QMPVW-D7KKK-3GKT6-VCFB2
$BinKey = [BinaryKey]::new('NW6C2-QMPVW-D7KKK-3GKT6-VCFB2')

# Allocate space for 64+ characters (plus null terminator)
$bufferSize = 128
$OutStr = New-IntPtr -Size ($bufferSize * 2)
$values = $BinKey.Group, $BinKey.Serial, $BinKey.Security, -7066209111506718658, $OutStr, $bufferSize
Write-Host
Write-Host "--- Encode Values ---`n" -ForegroundColor Yellow
Write-Host "Group ID    : $($values[0])"
Write-Host "Serial      : $($values[1])"
Write-Host "Security ID : $($values[2])"
Write-Host "HWID        : $($values[3])"
$dll = Join-Path $PSScriptRoot "MsftHelperDll.dll"
$result = Invoke-UnmanagedMethod -Dll $dll -Function GetInstallationIdString -Values $values
...
$generatedIid = ''
$result = [MSFT]::GetInstallationIdString($values[0], $values[1], $values[2], $values[3], [ref]$generatedIid)
...
if ($pythonCmd) {
  $Results = & python $iid2009 encode $values[0] $values[1] $values[2] $values[3]
  Write-Host "Python ID Script results : $Results" -ForegroundColor Green
}
...
[BitConverter]::GetBytes([Int32]$values[0]).CopyTo($buffer, 0x18)
[BitConverter]::GetBytes([Int32]$values[1]).CopyTo($buffer, 0x20)
[BitConverter]::GetBytes([Int64]$values[2]).CopyTo($buffer, 0x28)

# v25 = sub_180006A94((__int64)v12 + 8
$hBuffer = New-IntPtr -Size $bufSize
[Marshal]::Copy($buffer, 0, $hBuffer, $bufSize)

$pOutString = ''
$signatureBase = [IntPtr]::Add($hBuffer, 0x8)
$params = $signatureBase, $values[3], 0L, [ref]$pOutString
$dll = Join-Path $PSScriptRoot "pidgenxIn.dll"
Invoke-UnmanagedMethod `
    -Dll $dll `
    -Function "Inner" `
    -Values $params `
    -Sub 0x180006738 | Out-Null
Free-IntPtr $hBuffer -Method Auto

Write-Host "Pidgen  Dll Call results : $($pOutString)" -ForegroundColor Green
Free-IntPtr $OutStr

$pDecodedParams = New-IntPtr -Size 24
$Values = $generatedIid, $pDecodedParams
$dll = Join-Path $PSScriptRoot "MsftHelperDll.dll"
$readResult = Invoke-UnmanagedMethod -Dll $dll -Function ReadParametersFromString -Values $Values
...
if ($readResult -eq 0) {
    ...
    Write-Host
    Write-Host "--- Native DLL Decoded Successfully ---`n" -ForegroundColor Yellow
...
if ($readResult -eq 0) {
    ...
    Write-Host
    Write-Host "--- .Net C# Decoded Successfully ---`n" -ForegroundColor Yellow
...
if ($pythonCmd) {
  Write-Host
  Write-Host "--- Python IID Decoded Results ---" -ForegroundColor Yellow
  Write-Host
  & python $iid2009 decode $generatedIid
}
...
if ($pythonCmd) {
  Write-Host
  Write-Host "--- Python IID Recover Results ---" -ForegroundColor Yellow
  Write-Host
  & python $iid2009 recover $generatedIid $BinKey.CdKey.Substring(24,5)
}

Write-Host
Write-Host "In <> Out Test .. . . " -ForegroundColor Magenta
...
$HWID = 0L
$decodedParams = [Activator]::CreateInstance([Type]'DecodedParameters')
$readResult    = [MSFT]::ReadParametersFromString($OfflineId, [ref]$decodedParams);
if ($readResult -eq 0) {
...
## MSFT ##
<#

#>
## END ##